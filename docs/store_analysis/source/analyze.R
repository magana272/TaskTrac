...
store_levels <- c("JSON", "DUCKDB", "PARQUET", "REDIS")
...
trial <- read.csv("results/trial_data.csv", stringsAsFactors = FALSE) %>%
  mutate(
...
p_total <- ggplot(
  total_agg,
  aes(N, Total_ms, color = Store)
) +
  geom_line(linewidth = 0.9) +
  geom_point(size = 3) +
  scale_color_manual(values = store_colors) +
  scale_x_bench +
  scale_y_bench(max(total_agg$Total_ms, na.rm = TRUE)) +
  labs(
    title = "Total Create Time",
    subtitle = "Wall-clock time to insert N tasks",
    x = "N",
    y = "Total time (ms)"
  ) +
  theme_bench
